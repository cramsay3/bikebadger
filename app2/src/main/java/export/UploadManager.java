...
import db.DBHelper;
import export.Uploader.AuthMethod;
import export.Uploader.Status;
import feed.FeedList;
import us.theramsays.bikebadger.app2.BuildConfig;
...
public class UploadManager {
    ...
    public long load(String uploaderName) {
        String from[] = new String[] {
                "_id", Constants.DB.ACCOUNT.NAME, Constants.DB.ACCOUNT.AUTH_CONFIG, Constants.DB.ACCOUNT.FLAGS
        };
        String args[] = {
            uploaderName
        };
        Cursor c = mDB.query(Constants.DB.ACCOUNT.TABLE, from, Constants.DB.ACCOUNT.NAME + " = ?",
    ...
    private void handleAuthComplete(Uploader uploader, Status s) {
        ...
        switch (s) {
            ...
            case OK: {
                ...
                tmp.put(Constants.DB.ACCOUNT.AUTH_CONFIG, uploader.getAuthConfig());
                String args[] = {
                    Long.toString(uploader.getId())
                };
                mDB.update(Constants.DB.ACCOUNT.TABLE, tmp, "_id = ?", args);
                cb.run(uploader.getName(), s);
                return;
            }
            case NEED_AUTH:
    ...
    private void uploadOK(Uploader uploader, ProgressDialog copySpinner, SQLiteDatabase copyDB,
        ...
        ContentValues tmp = new ContentValues();
        tmp.put(Constants.DB.EXPORT.ACCOUNT, uploader.getId());
        tmp.put(Constants.DB.EXPORT.ACTIVITY, id);
        tmp.put(Constants.DB.EXPORT.STATUS, 0);
        copyDB.insert(Constants.DB.EXPORT.TABLE, null, tmp);
    }
    ...
    public void clearUpload(String name, long id) {
        ...
        if (uploader != null) {
            ...
            mDB.delete(Constants.DB.EXPORT.TABLE, Constants.DB.EXPORT.ACCOUNT + " = ? AND " + Constants.DB.EXPORT.ACTIVITY
                    + " = ?", args);
        }
    }
    ...
    public void loadLiveLoggers(List<Uploader> liveLoggers) {
        ...
        Cursor c = mDB.query(Constants.DB.ACCOUNT.TABLE, from,
                "( " + Constants.DB.ACCOUNT.FLAGS + "&" + (1 << Constants.DB.ACCOUNT.FLAG_LIVE) + ") != 0",
